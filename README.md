...
Данный проект реализует систему классификации спама с использованием машинного обучения. Используется набор данных `spam_or_not_spam.csv`, который содержит тексты сообщений и метки (`spam` или `not spam`). Модель обучается на сбалансированных данных, чтобы повысить точность предсказаний для редких классов.
